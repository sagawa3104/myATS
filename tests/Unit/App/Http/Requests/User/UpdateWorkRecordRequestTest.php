...
class UpdateWorkRecordRequestTest extends TestCase
{
    /**
    ...
    {
        //Arrange
        $request = new UpdateWorkRecordRequest();
        $rules = $request->rules();
    ...
    {
        //Arrange
        $this->app->resolving(UpdateWorkRecordRequest::class, function ($resolved) use ($data) {
            $resolved->merge($data);
        });

        //Act
        try {
            app(UpdateWorkRecordRequest::class);
    ...
    public function 補完データ検証(array $data, $expected)
    {
        ...
        //Act
        try {
            $result = app(UpdateWorkRecordRequest::class)->all();
        } catch (ValidationException $e) {
            $result = false;
        }
